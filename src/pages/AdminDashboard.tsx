...
import { Users, Truck, Heart, Trash2, Edit, TrendingUp, Sparkles, Timer } from 'lucide-react';
        ...
        <div className="p-6">
          <div className="grid md:grid-cols-3 gap-6 mb-6">
            ...
            <div className="bg-purple-50 rounded-xl p-4 border border-purple-100">
              ...
              <p className="text-sm text-purple-900 leading-snug">{demand.insight}</p>
            </div>
          </div>
          <div>
            ...
            <div className="flex items-end gap-2 h-24">
              {demand.weeklyForecast.map(({ day, predicted }) => {
                const heightPct = Math.round((predicted / maxForecast) * 100);
                return (
                  <div key={day} className="flex-1 flex flex-col items-center gap-1">
                    <div className="w-full bg-emerald-500 rounded-t-sm"
              ...
              {users.map(u => (
                <tr key={u.id}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-emerald-100 text-emerald-800 capitalize">{u.role}</span>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{u.email}<br />{u.phone}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                    <button onClick={() => toast.error('Edit not implemented in demo')} className="text-blue-600 hover:text-blue-900 mr-4">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {donations.map(d => {
                const f = getFreshnessTime(d.createdAt, d.consumableHours);
                return (
                  <tr key={d.id}>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center gap-2">
                        <img src={d.image} alt={d.foodName} className="w-8 h-8 rounded object-cover" />
                        <div>
                          <div className="text-sm font-medium text-gray-900">{d.foodName}</div>
                          <div className="text-xs text-gray-500">{d.quantity}</div>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{d.donorName}</td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`inline-flex items-center gap-1 px-2 py-1 text-xs font-semibold rounded-full border ${f.color}`}>
                        <Timer className="w-3 h-3" /> {f.label}
                      </span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      <button onClick={() => deleteDonation(d.id)} className="text-red-600 hover:text-red-900">
                        <Trash2 className="w-4 h-4 inline" />
                      </button>
                    </td>
                  </tr>
